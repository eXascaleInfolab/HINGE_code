...
def prepare_data_for_evaluation_and_evaluate_on_multiple_gpus (model, test, epoch, n_roles, n_values, whole_train, whole_test, whole_valid, gpu_ids_splitted, output_queue, new_eval_method, list_of_roles_h_ids, list_of_roles_t_ids, n_roles_h, dataset_without_h_and_t):
...
def main():
    ...
    parser.add_argument('--new_eval_method', default='False', help='If true, it prints all the facts (default: False)' )
        ...
        for epoch in range(1, int(args.epochs)+1):
            ...
            for i in range(len(train)): #batch_number == i
                ...
                for batch_num in range(n_batches_per_epoch[i]):
                    ...
                    x_batch, y_batch = Batch_Loader(train_i_indexes, train_i_values, n_values, n_roles, role_val, args.batchsize, arity, whole_train[i], id2value, id2role, args.new_negative_sampling_h_and_t, roleH2roleT, args.new_batching_method, args.num_negative_samples, args.dataset_without_h_and_t)
