...
def replace_val(n_values, last_idx, role_val, arity, new_facts_indexes, new_facts_values, whole_train_facts, new_batching_method, num_negative_samples):
    """
    Replace values randomly to get negative samples
    """
    rmd_dict = role_val #role2values

    if new_batching_method == 'True':
...
def replace_role(n_roles, last_idx, arity, new_facts_indexes, new_facts_values, whole_train_facts, new_negative_sampling_h_and_t, roleH2roleT, new_batching_method, num_negative_samples, dataset_without_h_and_t):
    """
    ...
    for cur_idx in range(new_range): #loop batch size times
        ...
        if new_negative_sampling_h_and_t == 'True':
            if dataset_without_h_and_t == 'True':
                if role_ind==0 or role_ind==2: #replace column 0 and 2 with the same key
        ...
        while (tuple(tmp_array) in whole_train_facts): # check if the corrupted fact exists in the whole_train_facts (which contains also the permute facts)
            if new_negative_sampling_h_and_t == 'True':
                if dataset_without_h_and_t == 'True':
                    rnd_role = np.random.randint(0, n_roles)
                    if role_ind==0 or role_ind==2: #replace column 0 and 2 with the same key
                        tmp_array[0] = rnd_role
                        tmp_array[2] = rnd_role
                    else:
                        tmp_array[role_ind] = rnd_role
                else:
                    #our negative sampling for head and tail (PX_h and PY_t)
                    if role_ind==0 or role_ind==2:
                        random_head = random.choice(list(roleH2roleT))
...
def Batch_Loader(train_i_indexes, train_i_values, n_values, n_roles, role_val, batch_size, arity, whole_train_facts, indexes_values, indexes_roles, new_negative_sampling_h_and_t, roleH2roleT, new_batching_method, num_negative_samples, fifty_percent_prob_of_creating_neg_samples, dataset_without_h_and_t):
    ...
    [[1.]
     ...
     [1.]]
    '''
    if fifty_percent_prob_of_creating_neg_samples == 'True':
        val_role = random.uniform(0, 1) #fifty-fifty probability of generating negative samples for roles and values
        if val_role < 0.5:
            replace_val(n_values, last_idx, role_val, arity, new_facts_indexes, new_facts_values, whole_train_facts, new_batching_method, num_negative_samples)
        else:
            replace_role(n_roles, last_idx, arity, new_facts_indexes, new_facts_values, whole_train_facts, new_negative_sampling_h_and_t, roleH2roleT, new_batching_method, num_negative_samples, dataset_without_h_and_t)
    else: #orignal NaLP probibility of generating negative samples. Most of the cases it generates negative samples for values.
